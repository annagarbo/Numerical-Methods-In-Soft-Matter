...
int main() {
    ...
    double temperatures[] = {2.6};
    int sizes[] = {20,25, 50,75, 100,200,500};
    ...
    for (int t = 0; t < num_temperatures; ++t) {
        for (int s = 0; s < num_sizes; ++s) {
            ...
            int spins[500][500];  // La dimensione 100 è il massimo previsto
            initialize_spins(spins, N);

            // Creazione del file per i dati
            char filename[50];
            snprintf(filename, sizeof(filename), "metro_T%.1f_N%d.txt", temperatures[t], N);
            ...
            // Chiudere il file dopo la simulazione
            fclose(file);
        }
    }
