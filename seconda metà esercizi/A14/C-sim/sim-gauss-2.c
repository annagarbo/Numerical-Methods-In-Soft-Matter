...
/* === Gaussian nrs === */
double gauss1,gauss0; int igau=0;
/* ==================== */
...
inline double Noise(){
  if(igau){ igau=0; return gauss1;}
  else{ BoxMuller(&gauss0, &gauss1); igau=1; return gauss0;}
}
...
int main(int argn, char **argc) 
{
  ...
  printf("\nsyntax:",tt); 
  printf("\nG-1-pp  nstep_save  N L  Bx By  T  v_ini Nv v_x_dec  k kp  R eps  R0 eps0 [dt=%.4lf] [tt=%.0lf]",dt,tt); 
  printf("\n        ++++++++++  **********  -  ^^^^^^^^^^^^^^^^  ****  -----  -------\n"); 
